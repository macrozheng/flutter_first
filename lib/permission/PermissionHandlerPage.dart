...
class _PermissionHandlerPageState extends State<PermissionHandlerPage> {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text("PermissionHandlerPage"),
        actions: <Widget>[
          IconButton(
              icon: new Icon(Icons.settings),
              onPressed: () {
                new PermissionHandler().openAppSettings();
              })
        ],
      ),
      body: new Center(
...
class _PermissionState extends State<PermissionWidget> {
  ...
  void requestPermission(PermissionGroup permission) {
    ...
    requestFuture.then((Map<PermissionGroup, PermissionStatus> permissionRequestResult) {
      setState(() {
